    ...
    try {
        if (b4j_ws.readyState !== 1) {
            if (b4j_closeMessage === false) {
                window.console.error("conexión cerrada.");
                window.alert("Conexión cerrada, refresca la página para conectarte.");
...
function b4j_connect(absolutePath) {
    if (typeof WebSocket === 'undefined') {
        window.alert("WebSockets no son soportados por el navegador.");
        return;
    }
